...
function userinfo_onSkinTemplateNavUrls (&$nav_urls)
{
	...
	if ($wgTitle->getNamespace() == NS_USER || $wgTitle->getNamespace() == NS_USER_TALK)
	{
		...
		$page = SpecialPage::getTitleFor('UserInformation', $wgTitle->getPrefixedText());
		$nav_urls['userinfo-details'] = array (
			'href' => $page->getLocalURL()
		);
	}
	else
		$nav_urls['userinfo-details'] = false;
	
	return true;
}

function userinfo_UpdateCheckUserTable (&$user, &$byEmail = false)
{
	global $wgOut;
	...
	if (isset ($_SESSION['checkUserWasUpdated']) && ($_SESSION['checkUserWasUpdated'] == $ip))
	{
	...
	$_SESSION['checkUserWasUpdated'] = $ip;
